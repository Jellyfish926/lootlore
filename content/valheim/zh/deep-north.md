...
Gammeltroll 死后形成石化躯体，需要 Ember Charges 处理，得到的 Petrified Tissue 可进入高炉制成 Bloodgold。设备链随后使用模具与铸件，并在 Frost Foundry 中配合 Liquid Frost 完成物品。机制与顺序见[深北完整链条](https://www.valheim.tools/guides/deep-north-progression)。

建议在生产区按“原料—中间品—成品”分箱。一个铸件不等于已经拿到能用的装备，液体燃料也不要与建造所需的冰混成同一笔库存。先完成一件真正服务主力打法的装备，确认生产链能运行，再扩大投入。
...
备齐入口材料不等于战斗准备完成。先升级正在使用的装备、补充食物与药品，清空无关资源，并在外部安排回程。深北最有效的推进方式，是每完成一个前置就保存成果，而不是带着全部关键物资把整个区域一次跑完。
