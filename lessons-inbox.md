...
1. **中文 title/description 按字符数判长度会误判,改按显示宽度(1 汉字 ≈ 2 字符),不放松阈值。**
...
2. **tech-audit 把外部热链 og:image 当本地文件查,40/40 页误报 `og_image_file_missing`。**
...
3. **`check_content.py` 解析时跳过 `<header>` 内的全部内容,把 H1 包在 `<header>` 里会被判 H1=0。**
...
4. **总站里「整个游戏目录是单一非英语、没有语种子目录」时,`check_content` 的 LANG_ATTR 会整目录误报。**
...
6. **中文攻略按「拉丁词 + 汉字/2」折算后普遍达不到 800 词下限——需要站主定中文页的字数口径。**
