---
slug: "deep-north"
url: "/valheim/deep-north/"
title: "Valheim Deep North: Eternal Pyre to Endgame"
seoTitle: "Valheim Deep North: Eternal Pyre to Endgame | Valheim Guide"
description: "The 1.0 Deep North order of operations, linking Embers, Petrified Tissue, Bloodgold, moulds, the Intricate Key and the invasions into one workable route."
category: "Deep North"
language: "en"
checkedAt: "2026-09-17"
scope: "Vanilla standard survival; world modifiers, mods and later patches may change the rules"
type: "article"
entities: ["deep-north", "frigid-kiln", "frost-foundry"]
related: ["intricate-key", "kall", "traders"]
chineseCharacters: 742
sourceUrls: ["https://mobalytics.gg/gamebase/guides/valheim-deep-north-progression-guide", "https://www.pcgamer.com/games/survival-crafting/valheim-intricate-key/", "https://www.valheim.tools/guides/deep-north-progression"]
date: "2026-09-17"
updated: "2026-09-17"
reviewed: "2026-09-17"
gameVersion: "1.0.12"
draft: false
author: "Jellyfi"
---

# Valheim Deep North: Eternal Pyre to Endgame

The Deep North is not the usual "find a new ore, craft a full set" loop. Work it as four threads — resources, processing, dungeons and events — and build steady production before you chase the final target. Item names stay in English so they match any client language; translated names differ, but the relationships do not.

## Step one: collect Embers before you leave the Ashlands

After you beat Fader, craft the Eternal Pyre and use it to collect Embers. Those go into Ember Charges, which in turn unlock an important Deep North material chain. The prerequisites are set out in the [Deep North progression guide](https://mobalytics.gg/gamebase/guides/valheim-deep-north-progression-guide).

Keep your old base as your logistics hub and leave the Black Forge, blast furnace and similar equipment standing. Do not tear down the old production area to move house and then discover up north that you still need it. Check gear, food, healing and return materials before you go; on a first trip, aim only to establish a foothold.

## Step two: find the tunnels, and store cores and moulds separately

The Winding Tunnels are the exploration target here, and they hold Frost Cores and moulds. The Frigid Kiln needs 10 Frostcore (plus Stone×20 and Ice×5); the amounts for the Frost Foundry should be taken from the in-game recipe. These building requirements are covered in this [guide to the key and the processing stations](https://www.pcgamer.com/games/survival-crafting/valheim-intricate-key/).

Do not assume one dungeon will cover everything you need. Use map markers to separate "unexplored" from "fully searched", and put the important moulds in their own chest when you get home.

## Step three: open up Bloodgold and cold processing

When a Gammeltroll dies it leaves a petrified body that has to be processed with Ember Charges; the Petrified Tissue you get goes into the blast furnace to make Bloodgold. The chain then uses moulds and casts, and finishes items in the Frost Foundry together with Liquid Frost. The mechanics and order are covered in this [full Deep North chain](https://www.valheim.tools/guides/deep-north-progression).

Sort your production area into raw materials, intermediates and finished goods. A cast is not the same as usable equipment, and liquid fuel should not sit in the same pile as the ice you need for building. Finish one item that genuinely serves your main playstyle, confirm the chain runs, then scale up.

## Step four: treat the key dungeon and the invasion as one task

Craft the Intricate Key, enter Mörkhalla, deal with the Malicious Ice inside, then travel to the invasion location on the world map and collect Malicious Blood. The normal route to the final entrance needs three lots of blood, so you will repeat this loop — see the [Deep North progression guide](https://mobalytics.gg/gamebase/guides/valheim-deep-north-progression-guide).

Advance one round at a time. Confirm the team and the supplies are ready before you trigger an event, and afterwards record how many rounds you have done and where the materials are stored before you prepare the next key. Do not judge by "I have been in three times" — check the actual materials.

## Only then enter the Aesir Passage

Having the entry materials is not the same as being ready to fight. Upgrade the gear you are actually using, top up food and potions, clear out unrelated resources, and set up a return point outside. The most effective way through the Deep North is to lock in each prerequisite as you finish it, rather than carrying every critical item while you run the whole biome in one go.

## What to read next

- [Valheim Intricate Key: Mould to Mörkhalla](/valheim/intricate-key/)
- [Valheim Kall Fimbulbringer: Three-Phase Plan](/valheim/kall/)
- [Valheim Traders: Haldor, Hildir, Bog Witch](/valheim/traders/)
