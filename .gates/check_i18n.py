...
用法: python3 check_i18n.py <html_dir> --default en --locales de,es,fr,it,ja [--min-ratio 0.4] [--json]
      ...
      --games slug1,slug2,...  总站产物(lootlore)用:产物路径是 /<game-slug>/<locale>/<page>,
                不是子站自身产物的 /<locale>/<page>,所以语种目录不在 <html_dir> 根下,而在
                <html_dir>/<slug>/ 下。传 --games 后按每个 slug 单独跑一遍这道门禁(有
                <slug>/<default>/ 才是多语种站,没有就打印明确的单语种跳过信息,不再对总站根目录
                做一次全空的伪检查)。不传就是老行为:把 <html_dir> 当成唯一一个站的根。
"""
...
def check_one(root, a, label=""):
    ...
    if not base:
        ...
        print(f"{tag}主语种目录 {a.default}/ 下没有 html(该目录共 {n} 个 html,非 /{a.default}/ 结构)—— "
              f"单语种站不需要这道门禁,跳过")
        return 0, [], [], {}
    base_words = {}
    ...
    for loc in [x for x in a.locales.split(",") if x and x != a.default]:
        ...
        stubs = 0
        for k in missing: W.append(("MISSING", f"{label}/{loc}/{k}", "主语种有此页,该语种未生成"))
        for k in extra: W.append(("EXTRA", f"{label}/{loc}/{k}", "主语种没有此页"))
    ...
    if not a.json:
        tag = f"[{label.strip('/')}] " if label else ""
        print(f"check_i18n: {tag}主语种 {len(base)} 页 · " + " · ".join(f"{l}: {s['pages']}页/覆盖 {s['coverage']}/残页 {s['stubs']}" for l, s in stats.items()))
        for k, r, m in E: print(f"  ❌ {k:12} {r}  {m}")
        for k, r, m in W[:100]: print(f"  ⚠  {k:12} {r}  {m}")
        if len(W) > 100: print(f"  … 另 {len(W)-100} 条警告")
    return len(base), E, W, stats
...
def main():
    ...
    if a.games:
        slugs = [s for s in a.games.split(",") if s]
        all_E, all_W, all_stats, results = [], [], {}, {}
        for slug in slugs:
            sroot = os.path.join(root, slug)
            if not os.path.isdir(sroot):
                print(f"[{slug}] 目录不存在于 {root} 下,跳过"); continue
            n, E, W, stats = check_one(sroot, a, label=f"/{slug}")
            all_E += E; all_W += W
            if stats: all_stats[slug] = {"default_pages": n, "locales": stats}
            results[slug] = {"default_pages": n, "multi_locale": bool(stats)}
        if a.json:
            print(json.dumps({"games": results, "errors": all_E, "warnings": all_W}, ensure_ascii=False, indent=1))
        else:
            multi = [s for s, r in results.items() if r["multi_locale"]]
            single = [s for s, r in results.items() if not r["multi_locale"]]
            print(f"check_i18n 汇总:{len(slugs)} 个游戏目录 · 多语种 {len(multi)}({', '.join(multi) or '无'}) "
                  f"· 单语种/跳过 {len(single)}({', '.join(single) or '无'}) · 阻塞 {len(all_E)} · 警告 {len(all_W)}")
        sys.exit(1 if all_E else 0)

    n, E, W, stats = check_one(root, a)
    if a.json:
        print(json.dumps({"default_pages": n, "locales": stats, "errors": E, "warnings": W}, ensure_ascii=False, indent=1))
